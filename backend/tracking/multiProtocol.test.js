...
const { bcdImei, buildGt06Packet, buildGt06Location } = require('./testHelpers');
...
function run() {
  const nmpPayload = [
    'NMP', 'ABCD01A', '1.6.5', 'NR', '1', 'L', '888888888888999', 'MH01PB0000',
    '1', '24032019', '060122', '29.7599630', 'N', '77.6277844', 'E', '022.5',
    '320.55', '04', '183.5', '1.0', '0.3', 'INA Airtel', '1', '1', '12.5',
    '4.2', '0', 'C', '25',
  ].join(',');
  const decodedMh = decodeMaharashtraPacket(Buffer.from(withChecksum(nmpPayload), 'ascii'));
  ...
  assert.strictEqual(encodeGt06Ack(PROTOCOL_LOGIN, 7).subarray(0, 2).toString('hex'), '7878');
  console.log('OK GT06 login packet decodes and ACK encodes');

  const location = buildGt06Packet(PROTOCOL_LOCATION, buildGt06Location({ lat: 19.1383, lng: 77.3210 }), 8);
  ...
  assert.ok(Math.abs(decodedLocation.records[0].longitude - 77.3210) < 0.000001);
