const { getLatestPositionForVehicle } = require('../services/trackingService');
...
exports.getLatestPosition = async (req, res) => {
  try {
    ...
    if (!latest) {
      return res.status(404).json({ success: false, message: 'Vehicle not found or not accessible' });
    }
    ...
    res.json({
      ...
      message: latest.position ? 'Latest position found' : 'No tracking data received yet',
    });
  } catch (err) {
